...
//triangle functions
bool ttest(Point a, Point b, Point c){
...
double tperimeter(Point a, Point b, Point c){
    return (distn(a,b)+distn(b,c)+distn(c,a));
}

double tarea(double a, double b, double c, double p){
    return sqrt(p*(p-a)*(p-b)*(p-c));
}

void tmain(Point p1, Point p2, Point p3) {
    double a,b,c,p;
    double dist;
    ...
    else
    {
        ...
        cout<<"Triangle can be formed, Perimeter would be: "<<p<<endl<<"Area: "<<tarea(a,b,c,p/2)
        <<endl;
...
//Quad functions
bool qtest(Point a, Point b, Point c, Point d){
    int abb=0;
    if(a.getx()*(b.gety()-c.gety())+b.getx()*(c.gety()-a.gety())+ c.getx()*(a.gety()-b.gety())==0)
        abb=abb+1;
...
double qarea(Point a, Point b, Point c, Point d){
...
void qmain(Point p1,Point p2, Point p3, Point p4) {
    double a,b,c,d,p;
    double dist;
    a=distn(p1,p2);
    b=distn(p2,p3);
...
string readFile(int input, string jj)
{
ifstream file(jj);
string in;
vector<string> lines;
